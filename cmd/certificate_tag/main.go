...
import (
	...
	"flag"
	"fmt"
	"github.com/scopevisio/windows-authenticode-cert-tagging/pkg"
	"os"
	"strings"
)

var (
	dumpAppendedTag       = flag.Bool("dump-appended-tag", false, "If set, any appended tag is dumped to stdout.")
	removeAppendedTag     = flag.Bool("remove-appended-tag", false, "If set, any appended tag is removed and the binary rewritten.")
	loadAppendedTag       = flag.String("load-appended-tag", "", "If set, this flag contains a filename from which the contents of the appended tag will be saved")
	setSuperfluousCertTag = flag.String("set-superfluous-cert-tag", "", "If set, this flag contains a string and a superfluous certificate tag with that value will be set and the binary rewritten. If the string begins with '0x' then it will be interpreted as hex")
	paddedLength          = flag.Int("padded-length", 0, "A superfluous cert tag will be padded with zeros to at least this number of bytes")
	savePKCS7             = flag.String("save-pkcs7", "", "If set to a filename, the PKCS7 data from the original binary will be written to that file.")
	outFilename           = flag.String("out", "", "If set, the updated binary is written to this file. Otherwise the binary is updated in place.")
	printTagDetails       = flag.Bool("print-tag-details", false, "IF set, print to stdout the location and size of the superfluous cert's Gact2.0 marker plus buffer.")
)
